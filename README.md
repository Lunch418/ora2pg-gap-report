...
- **`medium`** — не блокирует миграцию и не теряет данные, но реальное
  ...
  данные, а ранняя диагностика ошибочного явного присваивания), и
  отдельно `dbms_utl_calls` (намеренно широкий классификатор — реальное
  влияние конкретного вызова слишком разное, чтобы утверждать `high`
  для всех разом не покривив душой).
