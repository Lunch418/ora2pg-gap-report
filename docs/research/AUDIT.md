...
кода (`mortenbra/alexandria-plsql-utils` + `oracle-samples/db-sample-schemas`)
во время подготовки этого реестра — и для каждого из этих пяти в дереве
тестов лежит постоянный регрессионный тест, встраивающий реальный
фрагмент того самого исходника (не гипотетический пример), так что
находка остаётся проверяемой в любой момент, а не только "было замечено
в сессии однажды".
