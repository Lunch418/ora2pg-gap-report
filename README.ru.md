...
Всё выше — управляется флагами, намеренно: именно это делает инструмент
скриптуемым и удобным для CI. Для тех, кому проще смотреть глазами, чем
помнить флаги, `--tui` открывает экран с управлением мышью или
клавиатурой: выбор файла/директории в дереве, severity/язык, запуск
сканирования, клик по строке в таблице находок — открывает полное
объяснение (сообщение, `GAP-NNN`, и когда конкретно ломается — та же
информация, что уже показывают `--explain` и терминальный отчёт, просто
по клику):

```sh
pip install "ora2pg-gap-report[tui]"   # добавляет textual — не входит в базовую установку
ora2pg-gap-report --tui                # откроется в текущей директории
ora2pg-gap-report --tui path/to/schema_dump/   # или в указанной
```

![ora2pg-gap-report --tui — сканирование, сравнение с baseline и полное объяснение находки](docs/tui_demo.ru.gif)

Самостоятельный режим, как `--explain`/`--verify`: CLI-вызов принимает не
больше одного пути (стартовая точка в дереве, не список для сканирования —
выбор того, что сканировать, происходит уже внутри) и не сочетается с
флагами, формирующими скан (`--severity`, `--format`, `--fail-on`,
`--save` и остальные внутри TUI ничего не значили бы, поэтому сочетание
явно отклоняется, а не молча игнорируется). Внутри сам экран покрывает
то же самое, что и обычный флаговый workflow: кнопка «Add to selection»
добавляет в очередь больше одного файла/директории перед сканированием,
галочка «Check CONNECT BY» запускает ту же опциональную проверку через
ora2pg, что и `--check-connect-by`, а поле для baseline-файла — то же,
что `--save`/`--baseline`: экран результатов показывает счётчики
NEW/RESOLVED/UNCHANGED (и своей кнопкой «Save baseline» умеет сохранить
снапшот), а галочка «Verify mode» запускает то же пост-миграционное
сравнение, что и `--verify`. Запуск `--tui` без установленного extra
`[tui]` печатает понятную подсказку по установке, а не трейсбек.
...
Коды возврата специально различаются — чтобы CI мог отличить настоящий
результат от сломавшегося прогона:

| Код | Что значит |
| --- | --- |
| `0` | Скан прошёл, гейт (если был) пройден |
| `1` | Гейт `--fail-on` не пройден — есть находки этого уровня или выше |
| `2` | Ошибка использования или часть входных данных не просканирована (файл не найден/не читается, пустая директория, битый baseline) |
| `3` | Внутренняя ошибка — баг инструмента, а не находка миграции. Скан продолжается мимо упавшего детектора и выдаёт всё остальное, но прогон неполон, и `--save` пропускается |

Код `3` важнее всего в CI: упавший анализатор раньше возвращал `1` —
ровно то же, чем гейт сообщает, что честно отработал и нашёл проблемы.
...
- **Часть конструкций `ora2pg` копирует в вывод как есть** (`cross_apply`,
  `json_table`, `identity_column` и ещё 11) — для них повторный
...
- **Часть `ora2pg` молча выбрасывает или переписывает во что-то другое**
  (`read_only_table`, `table_partitioning`, ещё 20) — конструкции в выводе
  нет *по определению*, независимо от того, починил ли кто-то проблему
  вручную другим способом. Для них честный статус — `NOT_VERIFIABLE`, а не
...
К каждому GitHub Release также автоматически прикладывается базовый
бандл (без `--oracle`) как готовый файл-вложение — собран тем же
способом в CI, для тех, кому не нужно запускать скрипт самостоятельно.
