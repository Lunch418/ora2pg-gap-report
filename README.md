...
25 из 28 детекторов — `high`, 3 — `medium` (`context_object`,
`dbms_utl_calls`, `invisible_index`), `low` в реестре предусмотрен
(`--severity low`, диапазон часов в `effort_estimator.py`), но пока не
присвоен ни одним детектором — это честно, не потому что критерий не
придуман, а потому что ни один из подтверждённых 28 gap'ов в него не
попал. Не распределение ради распределения — так сложилось из реальных
находок, и вот по какому принципу:
...
Детекторов сейчас 28 (таблица выше), и почти все они устроены одинаково:
