...
from ..plsql_lex import IDENTIFIER, PACKAGE_BODY_NAME_RE, ROUTINE_START_RE, line_at, mask_strings_and_comments
...
_PACKAGE_VAR_RE = re.compile(
    rf"^[ \t]*({IDENTIFIER})\s+(?:(?:{_SCALAR_TYPES})(?:\s*\([^)]*\))?|{IDENTIFIER}(?:\.{IDENTIFIER})?%TYPE)"
...
def find_package_state(source: str) -> list[Finding]:
    """Detect a scalar variable declared at PACKAGE BODY top level (not
    inside any member routine's own declare section) -- Oracle's
    session-scoped package state. ora2pg rewrites reads/writes of such a
    variable into current_setting()/set_config() calls against a custom
    GUC parameter, but the rewrite has two confirmed bugs: set_config()
    is called with no explicit ::text cast (breaks on the very first
    call for any non-text variable), and current_setting() has no
    missing_ok => true (breaks differently from Oracle's own NULL-
    default semantics for a variable read before ever being set in the
    same session). See docs/research/gap-036-package-state.md.

    The package's own top-level declare section is bounded by whichever
    comes first: the next ROUTINE_START_RE match after the package's own
    IS/AS (its first member routine), or the *next* package body match
    (statement_end()-style bounding, same reasoning as read_only_table.py
    -- without this, a package with no member routines at all would let
    the declare-section search run unbounded into a later, unrelated
    package's own declarations, misattributing its variables to the
    wrong package). Everything before that boundary is package-level
    state -- a deliberate simplification for a heuristic tool: Oracle
    allows interleaving declarations and routines within a package body,
    but that ordering is rare in practice, and declarations after the
    first member routine are out of scope here rather than risked as a
    false positive.

    Uses PACKAGE_BODY_NAME_RE (plsql_lex's canonical package-body
    matcher, EDITIONABLE/NONEDITIONABLE included) rather than a
    detector-local reimplementation, and looks up each package's own
    ...
    pkg_matches = list(PACKAGE_BODY_NAME_RE.finditer(clean))
    for i, pkg_match in enumerate(pkg_matches):
        is_as = _IS_AS_RE.search(clean, pkg_match.end())
        if is_as is None:
            continue

        next_pkg_start = pkg_matches[i + 1].start() if i + 1 < len(pkg_matches) else len(clean)
        first_routine = ROUTINE_START_RE.search(clean, is_as.end(), next_pkg_start)
        declare_end = first_routine.start() if first_routine else next_pkg_start
        ...
        for var_match in _PACKAGE_VAR_RE.finditer(declare_text):
            findings.append(
                Finding(
                    ...
                    line=line_at(clean, is_as.end() + var_match.start()),
