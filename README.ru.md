...
Самостоятельный режим, как `--explain`/`--verify`: принимает не больше
одного пути (стартовая точка в дереве, не список для сканирования — выбор
того, что сканировать, происходит уже внутри) и не сочетается с флагами,
формирующими скан (`--severity`, `--format`, `--fail-on`, `--save` и
остальные внутри TUI ничего не значили бы, поэтому сочетание явно
отклоняется, а не молча игнорируется). Пока недоступно из TUI:
`--save`/`--baseline`/`--verify`, `--check-connect-by`, сканирование
больше одного пути за раз — для этого пока обычный вызов CLI. Запуск
`--tui` без установленного extra `[tui]` печатает понятную подсказку по
установке, а не трейсбек.
