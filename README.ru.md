...
на которых находка подтверждена (сейчас 25.0/16 у всех 105 — единая
...
сообщения об ошибках и `--help` (он следует `--lang` и тому же порядку
приоритетов выше). Research-документы `docs/research/` есть на обоих
языках — английский текст в `gap-NNN-*.md`, русский рядом, в `.ru.md`, —
и `--explain` печатает тот, что совпадает с языком вывода, а если
перевода нет, берёт другой (у GAP-002 и GAP-003 русской версии пока нет).
...
| `141` | Читатель закрыл канал (`\| head`, выход из `\| less`). Это вообще не результат скана: вывод оборвал читатель, и инструмент завершается молча. 128 + SIGPIPE — статус, который shell сообщает для процесса, убитого по SIGPIPE |
...
- **Часть конструкций `ora2pg` копирует в вывод как есть** (`cross_apply`,
  `json_table`, `identity_column` и ещё 48 — 51 из 106 детекторов) — для
...
- **Часть `ora2pg` молча выбрасывает или переписывает во что-то другое**
  (`read_only_table`, `table_partitioning` и ещё 52 — 54 из 106) —
...
Какой режим у какого детектора и почему, по 67 Oracle-gap'ам (режимы
детекторов MySQL и T-SQL — в `VERIFICATION_MODE` из `verification.py`) —
