...
## Итоговое распределение (47 gap'ов)

Обновлено после добавления GAP-038..047 (партия, найденная тем же
...
1. **`conversion` действительно ни разу не понадобился** — 0 из 47, даже
...
2. **`compile` как отдельная стадия по-прежнему не нужна** — ни один из
   ...
   в дампе `ora2pg` (упомянуто явно минимум в 10 из 47 research-документов
