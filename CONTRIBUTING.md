...
2. Перед PR: `pytest`, `ruff check`, `python3 scripts/doctor.py` — все три
   зелёные (`doctor.py` — часть CI, ловит расхождение реестра/документации
   с кодом на диске).
...
CLI-обёртки). Комментарии — только там, где объясняют неочевидное решение
